...
export default function SpeedParticles() {
    ...
    useFrame(({ camera }, delta) => {
        ...
        speed.start(vehicle.chassisBody.velocity.z)
        ...
        for (const [index, { position, velocity, scale }] of particles.entries()) {
            ...
            const speedScale = clamp(speed.get() / 6, 0, 1) ** 2 * 1.5
            ...
            if (position.z < vehicle.chassisBody.position.z || position.y < -4.5) {
                position.z += random.float(25, 30)
                position.y = vehicle.chassisBody.position.y + random.float(-4, 4)
